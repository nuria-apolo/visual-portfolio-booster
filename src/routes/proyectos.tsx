      ...
      <div className="mx-auto max-w-[1180px] overflow-hidden rounded-[28px] border border-black/8 bg-[#e9eaed] shadow-[0_20px_80px_rgba(0,0,0,.06)]">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <a
              href="/"
              className="rounded-xl border border-black/8 bg-white px-4 py-2 text-xs shadow-sm"
            >
              ← Inicio
            </a>
            <a
              href="/#contacto"
              className="rounded-xl bg-[#171717] px-4 py-2 text-xs font-medium text-white"
            >
              Hablemos
            </a>
          </div>
        ...
        <main className="space-y-3 p-3 pt-0">
          <header className="rounded-[24px] border border-black/8 bg-white px-6 py-14 sm:px-12 sm:py-20 lg:px-16">
            ...
            <p className="mt-8 max-w-2xl text-base leading-relaxed text-black/55 sm:text-lg">
              Identidad, tipografía y producto digital. Una selección de trabajos donde la
              estrategia se vuelve lenguaje, sistema y experiencia.
            </p>
          </header>

          <section
            aria-label="Todos los proyectos"
            className="rounded-[24px] border border-black/8 bg-white px-6 py-8 sm:px-12 sm:py-12 lg:px-16"
          >
              ...
              {projects.map((project, index) => (
                <article key={project.num} className={index % 3 === 0 ? "md:col-span-2" : ""}>
                  <div className="group relative overflow-hidden rounded-[20px] bg-[#ececee]">
                    ...
                    />
                    <span className="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-1.5 text-xs backdrop-blur">
                      {project.year}
                    </span>
                  </div>
                  {project.detailImages.length > 0 && (
                    <div
                      className={`mt-3 grid gap-3 ${project.detailImages.length > 1 ? "grid-cols-2" : ""}`}
                    >
                      {project.detailImages.map((image, detailIndex) => (
                        <img
                          key={image}
                          src={image}
                          alt={`${project.title}, imagen de detalle ${detailIndex + 1}`}
                          loading="lazy"
                          className="aspect-[16/10] w-full rounded-[16px] bg-[#ececee] object-cover"
                        />
                      ))}
                    </div>
                  )}
                  ...
                  <a
                    href={project.href}
                    className="mt-5 inline-flex border-b border-black/30 pb-1 text-sm"
                  >
          ...
          <section className="rounded-[24px] bg-[#171717] px-6 py-14 text-white sm:px-12 sm:py-20 lg:px-16">
            ...
            <h2 className="mt-5 max-w-4xl text-[clamp(3rem,7vw,7rem)] font-medium leading-[0.96] tracking-[-0.065em]">
        ...
        <footer className="flex flex-col gap-4 px-7 py-8 text-xs text-black/45 sm:flex-row sm:items-center sm:justify-between">
          <p>© 2026 Núria López</p>
