...
function SectionNumber({ children }: { children: string }) {
  return <span className="karma-section-number">{children}</span>;
}
    ...
    <div className="karma-case-study">
      ...
      <main className="karma-case-study-main">
        <nav className="case-study-nav" aria-label="Secciones del proyecto">
          <a href="/proyectos" className="case-study-nav-back"><span aria-hidden="true">←</span> Proyectos</a>
          <div className="case-study-nav-links">
            {navigation.map(([id, label], index) => <a key={id} href={`#${id}`} className={index === 0 ? "is-active" : ""}>{label}</a>)}
          </div>
        </nav>

        <section className="karma-intro" id="intro">
          <div className="karma-intro-copy">
            <p className="karma-eyebrow">KARMA FINANCIERO · PRODUCT DESIGN · 2026</p>
            <h1>¿Cómo compartir dinero sin que todo se convierta en una cuenta pendiente?</h1>
            <div className="karma-prose">
              <p>Karma Financiero nace de una idea sencilla: cuando compartimos nuestra vida con otras personas, también acabamos compartiendo una parte de nuestra economía.</p>
              <p>Parejas, familias, pisos compartidos o viajes tienen algo en común: hay gastos, aportaciones, objetivos y decisiones que dejan de pertenecer únicamente a una persona.</p>
              <p>La mayoría de herramientas financieras están pensadas para controlar el dinero. Karma parte de otra pregunta: ¿cómo podemos entenderlo mejor cuando el dinero es compartido?</p>
            ...
            <p className="karma-services">Product Design · UX/UI · Brand · Strategy</p>
          </div>
          <AssetPlaceholder label="HERO / DASHBOARD PRINCIPAL" aspectRatio="1.18 / 1" className="karma-hero-placeholder" />
        </section>
